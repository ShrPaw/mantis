...
class EventManager:
    ...
    def __init__(self, config: EventEngineConfig | None = None):
        ...
        self._spe_layer_stats = self._init_spe_layer_stats()
        self._spe_evaluations: int = 0
        self._spe_emitted: int = 0
        self._spe_suppressed_dupes: int = 0
        self._spe_cooldown_hits: int = 0
        self._spe_partial_4: int = 0
        self._spe_partial_6: int = 0
        self._spe_full_8: int = 0
    ...
    def on_trade(self, price: float, qty: float, delta: float, timestamp: float) -> list[dict]:
        ...
        if self.spe is not None:
            try:
                ...
                # Track layer pass status for every evaluation
                self._track_spe_layer_pass(price, qty, delta, timestamp)
    ...
    def get_event_stats(self) -> dict:
        ...
        if self.spe is not None:
            ...
            result["spe"]["layer_stats"] = self.get_spe_layer_stats()
    ...
    def _track_spe_layer_pass(self, price: float, qty: float, delta: float, timestamp: float):
        ...
        try:
            ...
            trap_result = self.spe.trap.update(
                ...
                displacement_origin=disp_result.get("displacement_origin", 0),
                displacement_end=disp_result.get("displacement_end", 0),
            )
            ...
            direction = self.spe._determine_direction(
                ...
                trap_result.get("trap_type", ""),
            )
            l7_pass = False
            if direction:
                entry_result = self.spe.entry.calculate(
                    direction, disp_result.get("displacement_origin", 0),
                    disp_result.get("displacement_end", 0), timestamp
                )
            ...
            if direction and l7_pass:
                exit_result = self.spe.exit_calc.calculate(
                    ...
                    disp_result.get("displacement_origin", 0),
                    disp_result.get("displacement_end", 0), timestamp
                )
    ...
    def get_spe_layer_stats(self) -> dict:
        ...
        return {
            ...
            "partial_4_layer_passes": self._spe_partial_4,
            "partial_6_layer_passes": self._spe_partial_6,
            "full_8_layer_passes": self._spe_full_8,
